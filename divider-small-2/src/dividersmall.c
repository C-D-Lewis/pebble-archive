...
//1.X UUID - 32a50538-7ac6-4235-93bf-a26a73e043eb
...
/**
 * Thanks SmallRock Software Inc.
 * Source: http://forums.getpebble.com/discussion/comment/67237/#Comment_67237
 */
...
static void tick_callback(struct tm *t, TimeUnits unit_changed)
{
  ...
  if(seconds % 60 == 0 || setup == false)
  {
    ...
    snprintf(voltage_buff, sizeof("5.000V"), "%sV", floatToString(voltage_buff, sizeof("5.000V"), voltage));
  ...
  cl_animate_layer(inverter_layer_get_layer(electron_layer), layer_get_frame(inverter_layer_get_layer(electron_layer)), GRect(x, y, w, h), 1000, 0);
  // layer_set_frame(inverter_layer_get_layer(electron_layer), GRect(x, y, w, h));
}
...
static void window_load(Window *window) 
{
  ...
  //TextLayers
  day_layer = cl_init_text_layer(GRect(40, 71, 30, 30), GColorWhite, GColorClear, false, 0, FONT_KEY_GOTHIC_18, GTextAlignmentLeft);
  layer_add_child(window_get_root_layer(window), text_layer_get_layer(day_layer));

  hours_layer = cl_init_text_layer(GRect(60, 38, 30, 30), GColorWhite, GColorClear, false, 0, FONT_KEY_GOTHIC_18, GTextAlignmentLeft);
  layer_add_child(window_get_root_layer(window), text_layer_get_layer(hours_layer));

  minutes_layer = cl_init_text_layer(GRect(60, 105, 30, 30), GColorWhite, GColorClear, false, 0, FONT_KEY_GOTHIC_18, GTextAlignmentLeft);
  layer_add_child(window_get_root_layer(window), text_layer_get_layer(minutes_layer));

  voltage_layer = cl_init_text_layer(GRect(105, 60, 60, 30), GColorWhite, GColorClear, false, 0, FONT_KEY_GOTHIC_18, GTextAlignmentLeft);
